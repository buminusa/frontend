...
export default function SuperAdminCategoriesPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (item: Category) => <span className="font-medium text-gray-900">{getLocalizedCategoryName(item, lang)}</span>,
    },
    {
      key: "slug",
      label: t("dashboard.categories.slug"),
      render: (item: Category) => <span className="text-gray-500 font-mono text-xs">{item.slug || "-"}</span>,
    },
    {
      key: "createdAt",
      label: t("dashboard.common.created"),
      render: (item: Category) => (
        <span className="text-gray-500">{item.createdAt ? new Date(item.createdAt).toLocaleDateString("id-ID") : "-"}</span>
      ),
    },
    {
      key: "actions",
      ...
      render: (item: Category) => (
        <div className="flex items-center gap-1">
          <button onClick={() => openEditModal(item)} className="p-2 rounded-lg hover:bg-blue-50 text-gray-400 hover:text-blue-600 transition-colors" title={t("dashboard.common.edit")}>
            <Pencil size={14} />
          </button>
          <button onClick={() => handleDelete(item.id)} className="p-2 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-600 transition-colors" title={t("dashboard.common.delete")}>
            <Trash2 size={14} />
          </button>
        </div>
      ),
    ...
    <DashboardLayout basePath="/dashboard/super-admin" roleLabel="Super Admin" mainClassName="p-4 lg:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">{t("dashboard.categories.platformDescription")}</p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 lg:p-6 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">{editingCategory ? t("dashboard.categories.edit") : t("dashboard.categories.add")}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-4 lg:p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t("dashboard.categories.name")}</label>
                <input
                  type="text"
                  value={formName}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">{t("dashboard.categories.englishName")}</label>
                <input
                  type="text"
                  value={formNameEn}
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 px-4 py-2.5 text-sm font-medium text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors">{t("dashboard.common.cancel")}</button>
