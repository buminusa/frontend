  ...
  useEffect(() => {
    ...
    async function loadOrders() {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(result.message || "Gagal memuat riwayat order.");
        }
        ...
        if ((err as Error).name !== "AbortError") {
          setError(err instanceof Error ? err.message : "Gagal memuat riwayat order.");
        }
      } finally {
  ...
  if (isLoading) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <p className="text-sm text-gray-500">Memuat riwayat order...</p>
      </div>
    );
  }
